...
export function studentView(
  ...
  /**
   * This needs to be agreed upon by both the client & the server
   */
  const updPort = 12345;
  ...
  client.bind(updPort, () => {
    console.log(`Polling for broadcast messages @ port ${updPort}`);
  });
  client.on('listening', () => {
  ...
  client.on('message', (message, rinfo) => {
    [ws_host, ws_port] = message.toString().split(':');
    const ws = new WebSocket(`ws://${ws_host}:${ws_port}`);
    console.log('UDP BROADCAST', ws_host, ws_port);
    ...
    /**
     * bind the client to the UDP port to start listening
     */
  });
}
