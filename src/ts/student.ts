...
export function studentView(context: vscode.ExtensionContext, clientData: object) {
  ...
  vscode.workspace.onDidChangeTextDocument(e => {
    ...
    try {
      const encodedDataToUInt8Array = new TextEncoder().encode(e.document.getText());
      if (e.document.uri.scheme === 'file' && e.document.uri.fsPath === vscode.window.activeTextEditor?.document.uri.fsPath) {
    

        const filePath = path.join(context.extensionPath, 'src', 'files', fileName);
        ...
        // console.log("sent")
        const fileList = vscode.workspace.fs.readDirectory(vscode.Uri.file(path.join(context.extensionPath, 'src', 'files'))).then((e: any) => {
          ...
          e.forEach((ele: any, i: number) => {
            ...
            const filePath = vscode.Uri.file(path.join(context.extensionPath, 'src', 'files', fileName))
